...
public class CitoyenCRUDImpl implements ICitoyenCRUD {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(CitoyenCRUDImpl.class);
    ...
    public void createCitoyen(Citoyen citoyen) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setString(10, PasswordHashUtil.hashPassword(citoyen.getMotDePasse()));

            ps.setDate(11, citoyen.getDateCreation() == null
                    ? new java.sql.Date(System.currentTimeMillis())
                    : new java.sql.Date(citoyen.getDateCreation().getTime()));
            ...
            logger.error("Erreur lors de la création du citoyen CIN={}", citoyen.getCin(), ex);
            throw new RuntimeException("Erreur technique lors de la création du citoyen");
        }
    }
    ...
    public void deleteCitoyen(Long id) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.error("Erreur lors de la suppression du citoyen ID={}", id, ex);
            throw new RuntimeException("Erreur technique lors de la suppression du citoyen");
        }
    }
    ...
    public Citoyen getById(Long id) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            logger.error("Erreur récupération citoyen ID={}", id, ex);
            throw new RuntimeException("Erreur technique lors de la récupération du citoyen");
        }
        return null;
    }
    ...
    public Citoyen updateCitoyen(Citoyen citoyen) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            String pwd = citoyen.getMotDePasse();
            if (pwd != null && pwd.length() == 64) {
                logger.debug("Mot de passe déjà haché pour citoyen ID={}", citoyen.getIdCitoyen());
                ps.setString(10, pwd);
            } else if (pwd != null && !pwd.isEmpty()) {
                ps.setString(10, PasswordHashUtil.hashPassword(pwd));
            } else {
                ps.setString(10, getById(citoyen.getIdCitoyen()).getMotDePasse());
            }
            ...
            logger.error("Erreur mise à jour citoyen ID={}", citoyen.getIdCitoyen(), ex);
            throw new RuntimeException("Erreur technique lors de la mise à jour du citoyen");
        }
    }
}
