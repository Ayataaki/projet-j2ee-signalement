...
<%-- 	</jsp:include> --%>
	...
	<div class="main-content">
		<div class="page-header">
			...
			<span class="text-muted">Bienvenue, ${sessionScope.user.nomUtilisateur}</span>
		</div>
							...
							<c:otherwise>
								...
								<c:out value="${s.designation}"/>
									<div class="activity-item 
										<c:choose>
											<c:when test="${s.statut == 'FINAL'}">resolved</c:when>
											<c:when test="${s.statut == 'PROCESSING'}">in-progress</c:when>
										...
										<div class="d-flex">
											...
											<div class="flex-grow-1">
												<p class="mb-1">
													...
													<c:choose>
														<c:when test="${s.statut == 'FINAL'}">a été résolu</c:when>
														<c:when test="${s.statut == 'PROCESSING'}">est en cours</c:when>
